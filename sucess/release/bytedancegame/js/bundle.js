(function () {
   ...
   const loadFile =  [
       ...
       [
           ...
           ['light','res/LayaScene_start/Conventional/light.lh'],
   ...
   class GameUI$2 extends Laya.Scene {
       ...
       flyUpdate() {
           ...
           qiu.transform.rotate(new Laya.Vector3(-.001, 0,0 ), true);
           qiu.transform.rotate(new Laya.Vector3( 0,0,utl.takeSpeed.y * Math.PI / 180 / 50 ), true);
           // qiu.transform.rotate(new Laya.Vector3( 0,utl.takeSpeed.y * Math.PI / 180 / 100,0 ), true)
           box.transform.translate(new Laya.Vector3(0,  0,utl.takeSpeed.x/10000), true);
           let leng = Laya.Vector3.scalarLength(box.transform.position);
           if(leng<36.2){
               box.transform.translate(new Laya.Vector3(0,  0,-utl.takeSpeed.x/10000), true);
           }

           box.getChildByName('fly').transform.localRotationEulerX = utl.takeSpeed.x / 6;
           box.getChildByName('fly').transform.localRotationEulerY = -utl.takeSpeed.y / 2;
           // let leng = Laya.Vector3.scalarLength(box.transform.position)
           // if(leng<36){
       ...
       getFrame() {
           ...
           frameTimes$1.push({ x, y, z, rx, ry, rz, sx, sy });
       }
       downSpeed() {
           // if(utl.takeSpeed.z.toFixed(1)==.01){
           //     utl.takeSpeed.y=0
